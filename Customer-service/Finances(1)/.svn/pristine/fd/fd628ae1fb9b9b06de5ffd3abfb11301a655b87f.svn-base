	...
	function delActivity(){
		...
		for(var i =0 ;i<stuList.length;i++){
			if(i!=stuList.length-1){
				ids = ids +stuList[i].nid+",";
			}else{
				ids = ids +stuList[i].nid;
			}
		}
		var url = "${pageContext.request.contextPath }/back/admin/deleteNotice";
		$.post(
			url,
			{
				ids:ids,
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<!-- 新增系别 -->
						...
						<div class="form-group">
							<label for="url" class="control-label col-sm-3">公告内容</label>
							<div class="col-sm-8">
								<input type="text" name="ncontent" class="form-control" id="ncontent">
	            			</div>
						</div>
						<div class="form-group">
